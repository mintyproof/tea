...
namespace Tea {
    ...
    public:
        PlatformSDL() {
            ...
            this->graphics = PlatformGraphicsSDL(this);
            this->input    = PlatformInputSDL(this);
        }
        ...
        std::unique_ptr<PlatformGraphicsSDL> graphics;
        std::unique_ptr<PlatformInputSDL>    input;
    };

    class PlatformGraphicsSDL: PlatformGraphics {
    public:
        PlatformGraphicsSDL(PlatformSDL& platform): platform(platform) {
            this->gl = SDL_GL_CreateContext(this->platform.window);
            if (this->gl == nullptr) {
                std::cerr << "Error creating OpenGL context: " << SDL_GetError() << std::endl;
                exit(1);
            ...
            // Draw to fill the framebuffer, just once
            SDL_Surface* surf = SDL_GetWindowSurface(this->window);
            SDL_FillRect(surf, NULL, SDL_MapRGB(surf->format, 255, 0, 0));
        }
    ...
    class PlatformInputSDL: PlatformInput {
    public:
        PlatformInputSDL(PlatformSDL& platform): platform(platform) {}
